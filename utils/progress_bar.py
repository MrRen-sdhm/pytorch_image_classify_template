...
class ProgressBar(object):
    ...
    def __init__(self, mode, epoch=None, total_epoch=None, current_lr=None, current_loss=None, current_top1=None,
                    model_name=None, total=None, current=None, width = 20, symbol = ">", output=sys.stderr):
        assert len(symbol) == 1
        ...
        if self.__first_init:
            with open("./logs/%s_%d.txt" % (self.model_name, config.fold), "w") as f: # write
                print('Time:', time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), file=f)
    ...
    def done(self):
        ...
        with open("./logs/%s_%d.txt" % (self.model_name, config.fold), "a") as f: # append
